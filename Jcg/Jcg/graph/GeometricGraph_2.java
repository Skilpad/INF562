...
public class GeometricGraph_2 extends GeometricGraph<Point_<Kernel_2>> {
	 ...
	 */	
	public GeometricGraph_2(GeometricGraph<Point_<Kernel_2>> g) {
		this.vertices = new ArrayList<GraphNode<Point_<Kernel_2>>>();
		...
		int i=0;
		for(GraphNode<Point_<Kernel_2>> v: g.vertices) {
			List<GraphNode<Point_<Kernel_2>>> vNeighbors=v.neighbors;
			GraphNode<Point_<Kernel_2>> vertexToUpdate=this.vertices.get(i);
			for(GraphNode<Point_<Kernel_2>> u: vNeighbors) {
				int index=u.getTag();
	...
	public static GeometricGraph_2 getGraphFrom2DTriangulation(List<Point_2> vertices, List<Point_2[]> faces) {
		...
		HashMap <Point_<Kernel_2>,Integer> vert = new HashMap<Point_<Kernel_2>,Integer> ();

		GeometricGraph_2 graph=new GeometricGraph_2();

		int i=0;
		for (Point_<Kernel_2> p : vertices) {
			graph.addNode(p);
			vert.put(p, new Integer(i++));
		}    

		for (Point_<Kernel_2>[] f : faces) {
			...
			GraphNode<Point_<Kernel_2>> v1,v2 ;
